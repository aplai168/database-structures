define(['mocha', 'chai', 'underscore'], function(notMocha, chai, _){
  var instantiator, follows;
  var ensure = function(_instantiator){
    instantiator = _instantiator
    return {'follows': follows};
  };

  var follows = function(pattern, options, prototypeOfInstances){
    var patternIs = function(){
      return _(arguments).contains(pattern);
    };

    if(!patternIs('functional',
                  'functional-shared',
                  'prototypal',
                  'pseudoclassical')){
    ...
    describe(pattern + ' instantiation style', function(){
      ...
      it(doesOrDoesnt('reuse methods across multiple instances', reusesMethods), function(){
        ...
        _(instance).methods().forEach(function(methodName){
          assuming(reusesMethods, expect(instance[methodName])).to.be.equal(otherInstance[methodName]);
        });
      });
