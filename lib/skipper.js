define(['underscore'], function(){
  var skipper = {};
  skipper.variant = document.location.search.replace(/\?variant=/,'');

  skipper.setup = function(files, sourceDirs, defaultDir){

    // default selection
    var keys = _(sourceDirs).keys();
    if (!_(keys).contains(skipper.variant)){
      window.location = '?variant=' + (defaultDir || keys[0]);
    }

    makeLinks(sourceDirs);
    var pathFromFile = function(file){
      return 'src/' + skipper.variant + '/' + file;
    }
    var specFromFile = function(file){
      return 'spec/' + file.replace(/.js$/,'') + 'Spec.js';
    }

    var srcs = files.map(pathFromFile);
